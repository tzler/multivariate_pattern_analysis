function MVPDsearchlight_computations(subject,inputs,outputPath)

%% ######## Initialize ########
preprocModels = inputs.preprocModels;
...
for iSphere = 1:nSpheres  % FOR DEBUGGING
    ...
    if any(~cell2mat(cellfun(@size,data_sphere,'UniformOutput',false)))
        ...
        preprocessedData_sphere = MVPDsearchlight_applyRegionModels(data_sphere,regionModels_spheres);
        ...
        results = MVPDsearchlight_applyInteractionModels(preprocessedData,interactionModels);
    end
