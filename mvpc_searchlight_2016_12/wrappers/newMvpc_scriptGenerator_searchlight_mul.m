...
cd(scriptDir);
fid = fopen(sprintf('sbatch_newMvpc_parallel_searchlight_mul_%d.sh',iNode),'wt');
...
fprintf(fid,'cd %s\n',scriptDir);
fprintf(fid,'\n');
fprintf(fid,'chmod +x sbatch_newMvpc_single_searchlight_mul_%d.sh\n',iNode);
fprintf(fid,'mpiexec -n %d ./sbatch_newMvpc_single_searchlight_mul_%d.sh',nTasks,iNode);
...
cd(scriptDir);
fid = fopen(sprintf('sbatch_newMvpc_single_searchlight_mul_%d.sh',iNode),'wt');
    
fprintf(fid,'#!/bin/bash\n');
fprintf(fid,'\n');
fprintf(fid,['file_num=sbatch_newMvpc_searchlight_mul_$(printf ' char(34) '%s' char(34) ' $(($OMPI_COMM_WORLD_RANK + 1 + %d)))\n'],'%03d',(iNode-1)*parameters.slurm.cores_per_node);
...
for iSubject = 1:nSubjects
        scriptName = sprintf('sbatch_newMvpc_searchlight_mul_%03d.m',iSubject);
        fid = fopen(scriptName,'wt');
        fprintf(fid,'function  %s',scriptName(1:end-2));
        fprintf(fid,'\n cd(''%s'')', mvpcDir);
        fprintf(fid,'\n runSearchlight_viewpoint_mul(%d)',iSubject);
        fclose(fid);
end
