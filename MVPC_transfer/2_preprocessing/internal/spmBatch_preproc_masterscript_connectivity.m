function spmBatch_preproc_masterscript_connectivity(Cfg_preproc)

% 1. check if debug is still needed
% 2. set extension as an option?
...
for iSubject = 1:length(Cfg_preproc.subjectPath)
    ...
    spmBatch_preproc_sliceTimingCorrection(Cfg_sliceTimingCorrection);
    ...
    spmBatch_preproc_spatialRealignment(Cfg_spatialRealignment);
    ...
    spmBatch_preproc_coregistration(Cfg_coregistration);
    ...
    spmBatch_preproc_segmentation(Cfg_segmentation);
    ...
    spmBatch_preproc_normalization(Cfg_normalization);
    ...
    spmBatch_preproc_smoothing(Cfg_smoothing);
    ...
    spmBatch_preproc_probabilityMaskGen(Cfg_probabilityGenMask);
    ...
    spmBatch_preproc_probabilityNormalization(Cfg_probabilityNormalization);
    ...
    spmBatch_utils_maskBinarize(Cfg_maskBinerize,'liberal','individual');
    ...
    spmBatch_utils_maskBinarize(Cfg_maskBinerize,'conservative','all');
    ...
    spmBatch_utils_maskSwell(Cfg_maskSwell,-1); % positive values swell mask, negative erode mask.
    ...
    spmBatch_utils_maskBinarize(Cfg_maskBinerize,'liberal','concatenate');
    ...
    Cfg_motion.maskFilter = '(^mask_wholebrain)(.*\.img)'; % must be RegEx filter
