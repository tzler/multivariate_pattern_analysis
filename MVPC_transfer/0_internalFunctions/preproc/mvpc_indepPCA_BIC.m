...
for iRun=1:nRuns
    ...
    for jRun = 1:nRuns
        if jRun ~= iRun
            data_concat = vertcat(data_concat,data_center{jRun});
        end
    end
    % calculate svd
...
% calculate optimal number of components
   d = size(data_concat,2);   % d is the dimensionality of the data (nVoxels)
   eigenVal = zeros(1,d);
...
%            p(k) =  prod(eigenVal(1:k))^(-N/2)*(v^scalingFactor*N)^maxExp*v^(scalingFactor*(e2-maxExp))*N^(e3-maxExp);
   ...
   if nPCs>parameters.maxPCs
    ...
    if isfield(data{iRun},'inverse')
        ...
        data_mean{iRun}.inverse{end+1} = @(prediction) prediction*data_PCA{iRun}.V';
    else
        data_mean{iRun}.inverse{1} = @(prediction) prediction*data_PCA{iRun}.V';
    end
end
