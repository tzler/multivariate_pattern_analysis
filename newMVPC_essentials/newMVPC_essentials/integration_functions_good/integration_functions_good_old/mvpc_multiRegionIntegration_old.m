    ...
    for iRun = 1:nRuns
        for nHiddenNodes = 1:20
            ...
            for iIteration = 1:nIterations
                ...
                net_lin = integration_linear_train(training_inputs_scaled',training_targets_scaled,nHiddenNodes);
                varexpl_lin(iRoi,iRun,nHiddenNodes,iIteration) = integration_linear_test(net_lin,testing_inputs_scaled',testing_targets_scaled);
                % Train and test model with nonlinear integration of modalities
                net_nonlin = integration_nonlinear_train(training_inputs_scaled',training_targets_scaled,nHiddenNodes);
                varexpl_nonlin(iRoi,iRun,nHiddenNodes,iIteration) = integration_nonlinear_test(net_nonlin,testing_inputs_scaled',testing_targets_scaled);
                % Store
                net_lin_storage{iRoi,iRun,nHiddenNodes,iIteration} = net_lin;
                net_nonlin_storage{iRoi,iRun,nHiddenNodes,iIteration} = net_nonlin;
            end
