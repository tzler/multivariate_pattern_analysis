...
% Cfg_mvpcRoi.interactionModels(3).functionHandle = 'mvpc_multipleLinearRegression_indepPCA_classification';
% non-linear multivariate connectivity
for iNode = 1:20
    Cfg_mvpcRoi.interactionModels(iNode).label = sprintf('iconn_nnet%02d',iNode);
    Cfg_mvpcRoi.interactionModels(iNode).regionModel = 3;
    Cfg_mvpcRoi.interactionModels(iNode).functionHandle = 'mvpc_neuralNet_indepPCA_classification';
    Cfg_mvpcRoi.interactionModels(iNode).parameters.nNodes = iNode;
    Cfg_mvpcRoi.interactionModels(iNode).parameters.labelsTemplatePath = '/mindhive/saxelab3/anzellotti/facesVoices_art2/modality_labels/%s.mat';
end
...
system('cd /mindhive/saxelab3/anzellotti/facesVoices_art2/newMVPC_essentials_classification/newMVPC_wrappers_facesVoices');
 for iJob = 23:44
 system(sprintf('sbatch sbatch_newMvpc_parallel_interactions_%d.sh',iJob));
 end
