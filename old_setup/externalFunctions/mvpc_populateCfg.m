function [exit_script, Cfg_mvpcRoi] = mvpc_populateCfg(Cfg_mvpcRoi)
%%
...
for idx1 = 1:length(Cfg_mvpcRoi.regionModels)
    for idx2 = 1:length(Cfg_mvpcRoi.regionModels(idx1).steps)
        if ~isfield(Cfg_mvpcRoi.regionModels(idx1).steps(idx2),'parameters')
            Cfg_mvpcRoi.regionModels(idx1).steps(idx2).parameters = [];
        end
    end
end
...
% (ty) the following might not be necessary 
